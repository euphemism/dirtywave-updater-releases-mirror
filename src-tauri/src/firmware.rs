...
use crate::{
    events::frontend_events::{
        DownloadState, DownloadStatus, FlashingCycle, FlashingStatus, FlashingStatusPayload,
        StartFirmwareDownloadPayload, UploadState, UploadStatus,
    },
    serial::{self, tycmd::TyCmdListEntry},
            ...
            while let Some(chunk) = stream.next().await {
                ...
                if now.duration_since(last_progress_update) >= PROGRESS_UPDATE_INTERVAL {
                    ...
                    app_handle.emit_to(
                        "main",
                        "flashing-status",
                        FlashingStatusPayload {
                            cycle: FlashingCycle::Downloading,
                            status: FlashingStatus::Downloading(DownloadStatus {
                                bytes_downloaded,
                                log: None,
                                size,
...
pub fn start_firmware_download_handler(
    app_handle: Arc<AppHandle>,
    payload: StartFirmwareDownloadPayload,
) {
    ...
    tauri::async_runtime::spawn(async move {
        ...
        if let Some(serial_interface) = interfaces.iter().find(|interface| interface[0] == "Serial")
        {
            log::info!("Communicating on interface {:?}", serial_interface[1]);

            serial::get_m8_details(serial_interface[1].as_str());
        }
    });
    ...
    tauri::async_runtime::spawn(async move {
        ...
        let device = state_guard.devices.get(&payload.device).cloned();
        let version = state_guard.version.clone();
        ...
        let result = if let Some(device) = device {
            match download_firmware(&download_firmware_app_handle.clone()).await {
                Ok(firmware_paths) => {
                    let _ = download_firmware_app_handle.emit_to(
                        "main",
                        "flashing-status",
                        FlashingStatusPayload {
                            cycle: FlashingCycle::Uploading,
                            status: FlashingStatus::Uploading(UploadStatus {
                                log: Some(
                                    "upload@status Verifying firmware matches device".to_string(),
                                ),
                                    ...
                                    let _ = download_firmware_app_handle.emit_to(
                                        ...
                                        FlashingStatusPayload {
                                            ...
                                            status: FlashingStatus::Uploading(UploadStatus {
                                                ...
                                                state: if output
                                                    .contains(SENDING_RESET_COMMAND_SUBSTRING)
                                                {
